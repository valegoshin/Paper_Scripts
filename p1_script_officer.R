...
vif_fit3 <- car::vif(fit3) %>% as.data.frame() %>% 
  mutate(variable = rnames, 
         variable = str_replace_all(variable, "_", " ")) %>% 
  select(2, 1) %>% set_names("variable", "vif")%>%  
  regulartable() %>% 
  style(pr_t = fp_text(font.size = 9, font.family = "Arial Narrow"),
        pr_p = fp_par(text.align = "center"), part = "all") %>% 
  bold(part = "header") %>% theme_box() %>% height_all(0) %>% 
...
fit_glance_tab <- fit_glance %>% as.data.frame() %>% 
  mutate(patameter = rnames) %>% select(4, 1, 2, 3) %>% 
  regulartable() %>% 
  style(pr_t = fp_text(font.size = 9, font.family = "Arial Narrow"),
        pr_p = fp_par(text.align = "center"), part = "all") %>% 
  bold(part = "header") %>% theme_box() %>% height_all(0) %>% 
...
ncvt_df <- data.frame(test = ncvt$test,
